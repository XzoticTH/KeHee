...
async function getSession(){ return await chrome.storage.session.get({token:'',apiBase:'',user:null}); }
...
async function ensureOriginPermission(base){
  const u=validateApiUrl(base);
...
async function api(path, options={}){
  const s=await getSession(); const base=normalizeBase(apiBase||s.apiBase);
  if(!base) throw new Error('กรุณาระบุ API URL ของเซิร์ฟเวอร์');
...
async function showLogin(){ $('appView').classList.add('hidden');$('loginView').classList.remove('hidden');const s=await getSession();$('apiUrl').value=s.apiBase||localStorage.getItem('memberApiBase')||'';$('loginId').focus(); }
...
$('loginForm').addEventListener('submit',async e=>{
  ...
  apiBase=normalizeBase($('apiUrl').value); $('loginStatus').textContent='กำลังตรวจสอบ…';
  try{
    validateApiUrl(apiBase);
    const permitted=await ensureOriginPermission(apiBase); if(!permitted) throw new Error('ไม่ได้อนุญาตให้ Extension เชื่อมต่อเซิร์ฟเวอร์นี้');
    const d=await api('/login',{method:'POST',body:JSON.stringify({id,password})});
    await chrome.storage.session.set({token:d.token,apiBase,user:d.user});
    localStorage.setItem('memberApiBase',apiBase); $('loginPassword').value=''; $('loginStatus').textContent=''; await openApp(d.user);
...
(async()=>{const s=await getSession();apiBase=normalizeBase(s.apiBase||localStorage.getItem('memberApiBase')||'');if(s.token){try{const d=await api('/me');await openApp(d.user);return;}catch(_){}}await showLogin();})();
